    ...
    try:
        ...
        if authorized:
            ...
            if isinstance(content, StreamingResponse):
                # StreamingResponse의 경우 특별한 처리
                response = content
                response_data = "Streaming data, cannot log content directly."
            elif isinstance(content, Response):
                response = content
                response_data = "Response data, logged in memory for logging."
            else:
                response = JSONResponse(content=content, status_code=status.HTTP_200_OK)
                response_data = content if isinstance(content, dict) else str(content)
            ...
            response_data = response.body if not isinstance(response, StreamingResponse) else "Unauthorized or error response"
            processed_time = None
        return response
...
def add_log_to_background(request: Request, response: Response, processed_time: float):
    if isinstance(response, StreamingResponse):
        # StreamingResponse의 경우 특별한 로깅 처리
        content = "Streaming content cannot be logged directly"
    else:
        # 일반적인 응답의 경우 body를 로그로 기록
        content = response.body if not isinstance(response, StreamingResponse) else 'StreamingResponse content'
